...
# function to calculate the normalvector of a triangle/face. Used for normals of inner circles
# we use just the cross product of two of the defining edges, generated by the vertices
BindGlobal( "__SIMPLICIAL__CalculateMaxDistFromOrigin",
    function(surface,printRecord)
	local A, B, C, dist, res, normal, face;

    res := Float(0);
    for vertex in Vertices(surface) do
        dist := 0;
        dist := dist + GetVertexCoordinates3D(surface, vertex, printRecord)[1]*GetVertexCoordinates3D(surface, vertex, printRecord)[1];
        dist := dist + GetVertexCoordinates3D(surface, vertex, printRecord)[2]*GetVertexCoordinates3D(surface, vertex, printRecord)[2];
        dist := dist + GetVertexCoordinates3D(surface, vertex, printRecord)[3]*GetVertexCoordinates3D(surface, vertex, printRecord)[3];
...
InstallMethod( DrawSurfaceToJavaScriptCalculate,
    ...
    function(surface, fileName, printRecord, calculate)
              ...
              incenter,inradius, normal, atemp, btemp, material, maxDistFromOrigin;	
    ...
    for i in [1..Length(uniqueFaceColors)] do
        ...
        AppendTo(output, """
            ...
            material""",i,""".clipIntersection = false;
        """);
    ...
    # add intersection planes
    maxDistFromOrigin := __SIMPLICIAL__CalculateMaxDistFromOrigin(surface, printRecord);
    AppendTo(output, "const maxDist = ");
    AppendTo(output, maxDistFromOrigin);
    AppendTo(output, ";");        
